...
class test_generator:
    ...
    def run(self):
        ...
        for i in range(args.test_amount):
            ...
            generator.random_input_generate_file("input_generated.txt", args.number_stations, args.number_lines, args.number_trains, args.number_passengers, args.max_capacity_station,
                                                 args.max_capacity_line, args.max_length_line, args.max_capacity_train, args.max_groupsize_passenger, args.max_targettime_passenger, args.max_speed_train)

            if system() == "Linux":
                p1 = subprocess.run(
                    'python3 -m abfahrt < input_generated.txt', capture_output=True, shell=True)

            elif system() == "Windows":
                p1 = subprocess.run(
                    'python -m abfahrt < input_generated.txt', capture_output=True, shell=True)
            else:
                return
            ...
            if system() == "Linux":
                p2 = subprocess.run(
                    '"./abfahrt/simulator/bahn-simulator" -input input_generated.txt -output output.txt -verbose', stdout=subprocess.PIPE, shell=True)

            elif system() == "Windows":
                p2 = subprocess.run(
                    '"abfahrt/simulator/Bahn-Simulator.exe" -input input_generated.txt -output output.txt -verbose', stdout=subprocess.PIPE, shell=True)
            else:
                return
            if args.verbose:
