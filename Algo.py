...
class Algo:
	...
	def _get_fastest_train_with_capacity(self,route_data_list,passengers):
		...
		for route_data in route_data_list:
			if route_data[1] == -1: #there is no route for this train, so ignore this train (happens very uncommon)
				continue
			elif not travel_center.check_capacity_train(route_data[0],route_data[1],route_data[2],route_data[0],passengers):
				continue
	...
	def run(self):
		...
		while self.groups_manager.get_priority():
			...
			if train_center.callable_trains(start):
			
				trains_available = travel_center.callable_trains(start) #trains_available[n] = [train1:class Train,....]
				...
				for train in trains_available:
					...
					#travel_center.determine_route(train,start,end) -> [train:class Train,start_time:int,end_time:int]
                                        route_data_list.append(travel_center.determine_route(train,start_station,end_station))
				...
				if train_fastest_and_time[0] != None: #if there is a train with enough capacity
					...
					travel_center.save_route(train_fastest_and_time[0],start_station,end_station,group_with_prio)
			...
			trains_callable_not_in_move = travel_center.callable_trains_not_in_move(start_station) #trains_callable_not_in_move = [[train1:class Train,station:class Station],....]	
			trains_callable_in_move = travel_center.callable_trains_in_move(start_station,end_station) #trains_callable_in_move = [[train1:class Train,station:class Station],....]
					
			route_data_list_not_move = []
			route_data_list_move = []		
					
			for train_and_time in trains_callable_not_in_move:
				route_data_list_not_move.append(travel_center.determine_route(train_and_time[0],train_and_time[1],end))
					
			for train_and_time in trains_callable_in_move:
				route_data_list_move.append(travel_center.determine_route(train_and_time[0],train_and_time[1],end))
			...
			if train_max_fastest_and_time_not_move[0] != None and train_max_fastest_and_time_move[0] != None: #if train in and also not in move are available for calling           	
                                ...
                                if train_max_fastest_and_time_not_move[1] < train_max_fastest_and_time_move[1]:
                                        travel_center.call_train_not_in_move(train_max_fastest_and_time_not_move[0],start,end)
                                else:
                                        travel_center.call_train_in_move(train_max_fastest_and_time_move[0],start,end)

                        elif train_max_fastest_and_time_not_move[0] != None: #if only trains not in move are available for calling

                                travel_center.call_train_not_in_move(train_max_fastest_and_time_not_move[0],start,end,group_with_prio)

                        elif train_max_fastest_and_time_move[0] != None: #if only trains in move are available for calling

                                travel_center.call_train_in_move(train_max_fastest_and_time_move[0],start,end,group_with_prio)
                                ...
                                groups_manager.split_group(group_with_prio)
                                continue
			...
			travel_center.optimize_full_station(train_max_fastest_and_time_not_move[0],end) 				
